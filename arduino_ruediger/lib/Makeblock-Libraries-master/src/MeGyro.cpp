...
void MeGyro::update(void)
{
  ...
  */
  ...
  dmpGetQuaternion(&(this->q), i2cData);
...
uint8_t MeGyro::dmpGetQuaternion(int16_t *data, const uint8_t* packet) {
    // TODO: accommodate different arrangements of sent data (ONLY default supported now)
    // if (packet == 0) packet = dmpPacketBuffer;
    if (packet == 0) packet = 42;
...
uint8_t MeGyro::dmpGetQuaternion(Quaternion *q, const uint8_t* packet) {
    // TODO: accommodate different arrangements of sent data (ONLY default supported now)
    int16_t qI[4];
